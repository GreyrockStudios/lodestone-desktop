...
const TEMP_DIR = IS_WIN ? os.tmpdir() : "/tmp";
const APPS_DIR = IS_WIN ? path.join(process.env.ProgramFiles || "C:\\Program Files") : "/Applications";
const SYSTEM_DIRS = IS_WIN
  ? ["C:\\Windows", "C:\\Program Files", "C:\\Program Files (x86)"]
  : ["/System", "/Library/System", "/etc/shadow", "/etc/ssh", "/private/var/db/dslocal"];
...
function normalizePath(filePath) {
  // On Windows, LLMs often generate Unix-style paths like /home/user/Desktop/file.txt
  // Convert obvious Unix-style paths to Windows equivalents
  if (IS_WIN && filePath.startsWith("/")) {
    // /home/username/... → C:\Users\username\...
    const unixHomeMatch = filePath.match(/^\/home\/([^/]+)(.*)/);
    if (unixHomeMatch) {
      filePath = path.join("C:\\Users", unixHomeMatch[1], unixHomeMatch[2].replace(/\//g, "\\"));
    }
    // /tmp → Windows temp dir
    else if (filePath.startsWith("/tmp")) {
      filePath = path.join(os.tmpdir(), filePath.slice(4).replace(/\//g, "\\"));
    }
    // /Desktop/..., /Documents/..., /Downloads/... relative to home
    else if (filePath.startsWith("/Desktop/") || filePath.startsWith("/Documents/") || filePath.startsWith("/Downloads/")) {
      filePath = path.join(HOME, filePath.replace(/\//g, "\\"));
    }
    else {
      // Unknown Unix path on Windows — try resolving relative to home
      filePath = path.join(HOME, filePath.slice(1).replace(/\//g, "\\"));
    }
  }
