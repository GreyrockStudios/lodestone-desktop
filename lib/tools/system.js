...
function register(mainWindow, store, auditLog) {
    ...
    try {
      ...
      try {
        if (process.platform === 'win32') {
          const { stdout: diskOut } = await execFileAsync('powershell', ['-NoProfile', '-Command', "$d = Get-PSDrive -PSProvider FileSystem | Where-Object { $_.Root -eq (Split-Path $env:USERPROFILE -Qualifier) + '\\' }; [PSCustomObject]@{UsedGB=[math]::Round($d.Used/1GB,1);FreeGB=[math]::Round($d.Free/1GB,1);TotalGB=[math]::Round(($d.Used+$d.Free)/1GB,1)} | ConvertTo-Json"]);
          try {
            const parsed = JSON.parse(diskOut.trim());
            if (parsed) {
              diskInfo = { total: `${parsed.TotalGB}GB`, used: `${parsed.UsedGB}GB`, available: `${parsed.FreeGB}GB`, usePercent: `${Math.round(parsed.UsedGB / parsed.TotalGB * 100)}%` };
            }
          } catch { /* fallback */ }
        } else {
  ...
  ipcMain.handle("tool:browser-open", async (_e, url) => {
    // Validate URL — only http/https allowed (same as open-external)
    if (!url || typeof url !== 'string' || (!url.startsWith('http://') && !url.startsWith('https://'))) {
      auditLog("browser-open", url, "BLOCKED: non-http URL", store.get("file-access-tier"));
      return { error: "Only http/https URLs allowed" };
    }
    try {
      await shell.openExternal(url);
