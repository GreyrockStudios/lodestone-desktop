...
function register(mainWindow, store, auditLog) {
  ...
  ipcMain.handle("tool:wifi-info", async () => {
    try {
      ...
      if (process.platform === "darwin") {
        // Try en0 first, then en1 (Mac Mini has WiFi on en1)
        let wifiIface = null;
        for (const iface of ['en0', 'en1', 'en2']) {
          try {
            const result = execSync(`networksetup -getairportnetwork ${iface} 2>/dev/null`).toString().trim();
            if (!result.includes('not a Wi-Fi') && !result.includes('Error')) {
              wifiIface = iface;
              wifiInfo.network = result.replace('Current Wi-Fi Network: ', '');
              break;
            }
          } catch { /* not a wifi interface, try next */ }
        }
        if (!wifiIface) wifiInfo.network = 'No Wi-Fi adapter found';
        // Get IP from the WiFi interface or primary interface
        const ipIface = wifiIface || 'en0';
        try { wifiInfo.ip = execSync(`ipconfig getifaddr ${ipIface} 2>/dev/null`).toString().trim() || 'Unknown'; } catch { wifiInfo.ip = 'Unknown'; }
  ...
  ipcMain.handle("tool:active-window", async () => {
    try {
      ...
      if (process.platform === "darwin") {
        try {
          const appName = execSync(`osascript -e 'tell application "System Events" to get name of first application process whose frontmost is true'`, { timeout: 5000 }).toString().trim();
          let windowTitle = "unknown";
          try {
            windowTitle = execSync(`osascript -e 'tell application "${appName}" to get name of front window'`, { timeout: 5000 }).toString().trim();
          } catch { /* some apps don't expose window titles */ }
          windowInfo = { app: appName || "unknown", window: windowTitle };
