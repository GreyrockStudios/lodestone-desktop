...
function registerUpdateHandlers() {
  ...
  autoUpdater.on("update-downloaded", () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed())
      dialog.showMessageBox(mainWindow, { type: "info", title: "Update Available", message: "A new version of Lodestone is available.", detail: "It will be installed when you restart the app.", buttons: ["Restart Now", "Later"] }).then(({ response }) => { if (response === 0) { setIsQuitting(true); autoUpdater.quitAndInstall(); } });
  });
