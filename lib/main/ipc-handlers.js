...
function registerDbHandlers(db) {
  ...
  ipcMain.handle("db:create-conversation", (_e, data) => {
    ...
    if (data.title && typeof data.title === 'string' && data.title.length > 500) return { error: 'Title too long (max 500 chars)' };
  ...
  ipcMain.handle("db:add-message", (_e, data) => {
    if (!data || typeof data !== 'object') return { error: 'Invalid data' };
    if (data.content && typeof data.content === 'string' && data.content.length > 500000) return { error: 'Message content too long (max 500KB)' };
  ...
  ipcMain.handle("db:create-memory", (_e, data) => {
    if (!data || typeof data !== 'object') return { error: 'Invalid data' };
    if (data.content && typeof data.content === 'string' && data.content.length > 100000) return { error: 'Memory content too long (max 100KB)' };
  ...
  ipcMain.handle("db:create-commitment", (_e, data) => {
    if (!data || typeof data !== 'object') return { error: 'Invalid data' };
    if (data.description && typeof data.description === 'string' && data.description.length > 5000) return { error: 'Description too long (max 5000 chars)' };
    return db.createCommitment(data);
  });
  ipcMain.handle("db:update-commitment", (_e, id, data) => {
    if (!id || typeof id !== 'string') return { error: 'Invalid id' };
    return db.updateCommitment(id, data);
  });
  ...
  ipcMain.handle("db:create-folder", (_e, data) => {
    if (!data || typeof data !== 'object') return { error: 'Invalid data' };
    if (data.name && typeof data.name === 'string' && data.name.length > 255) return { error: 'Folder name too long (max 255 chars)' };
    return db.createFolder(data);
  });
  ...
  ipcMain.handle("db:create-artifact", (_e, data) => {
    if (!data || typeof data !== 'object') return { error: 'Invalid data' };
    if (data.content && typeof data.content === 'string' && data.content.length > 1000000) return { error: 'Artifact content too large (max 1MB)' };
    return db.createArtifact(data);
  });
  ipcMain.handle("db:update-artifact", (_e, id, data) => {
    if (!id || typeof id !== 'string') return { error: 'Invalid id' };
    if (data.content && typeof data.content === 'string' && data.content.length > 1000000) return { error: 'Artifact content too large (max 1MB)' };
    return db.updateArtifact(id, data);
  });
...
function registerBrainHandlers() {
  ...
  ipcMain.handle("brain:set-soul", (_e, content) => {
    if (typeof content !== 'string') return { error: 'Soul content must be a string' };
    if (content.length > 100000) return { error: 'Soul content too long (max 100KB)' };
    return brain.setSoul(content);
  });
  ipcMain.handle("brain:get-identity", () => brain.getIdentity());
  ipcMain.handle("brain:set-identity", (_e, data) => {
    if (!data || typeof data !== 'object') return { error: 'Invalid data' };
    return brain.setIdentity(data);
  });
  ipcMain.handle("brain:get-rules", () => brain.getRules());
  ipcMain.handle("brain:add-rule", (_e, rule, category, priority) => {
    if (typeof rule !== 'string' || rule.length > 5000) return { error: 'Rule text too long (max 5000 chars)' };
    return brain.addRule(rule, category, priority);
  });
  ...
  ipcMain.handle("brain:set-heartbeat", (_e, data) => {
    if (!data || typeof data !== 'object') return { error: 'Invalid data' };
    return brain.setHeartbeat(data);
  });
  ipcMain.handle("brain:get-user-profile", () => brain.getUserProfile());
  ipcMain.handle("brain:set-user-profile", (_e, data) => {
    if (!data || typeof data !== 'object') return { error: 'Invalid data' };
    return brain.setUserProfile(data);
  });
  ...
  ipcMain.handle("brain:ingest-memories", (_e, extracted) => {
    if (!extracted || typeof extracted !== 'object') return { error: 'Invalid data' };
    return brain.ingestMemories(extracted);
  });
  ...
  // Self-improvement
  ipcMain.handle("brain:create-prediction", (_e, data) => {
    if (!data || typeof data !== 'object') return { error: 'Invalid data' };
    return brain.createPrediction(data);
  });
  ...
  ipcMain.handle("brain:learn-from-correction", (_e, correction) => {
    if (!correction || typeof correction !== 'object') return { error: 'Invalid data' };
    return brain.learnFromCorrection(correction);
  });
...
function registerSchedulerHandlers() {
  ...
  ipcMain.handle("scheduler:create", (_e, task) => {
    if (!task || typeof task !== 'object') return { error: 'Invalid task data' };
    // Validate cron expression: must be 5 fields (standard) or 6 fields (with seconds)
    if (task.cron_expr) {
      const parts = task.cron_expr.trim().split(/\s+/);
      if (parts.length < 5 || parts.length > 6) return { error: 'Invalid cron expression: must be 5 or 6 fields' };
    }
    // Validate task_type if provided
    const VALID_TASK_TYPES = ['reminder', 'message', 'brain_task', 'custom'];
    if (task.task_type && !VALID_TASK_TYPES.includes(task.task_type)) return { error: `Invalid task_type: ${task.task_type}` };
  ...
  ipcMain.handle("scheduler:update", (_e, id, updates) => {
    ...
    if (updates.cron_expr) {
      const parts = updates.cron_expr.trim().split(/\s+/);
      if (parts.length < 5 || parts.length > 6) return { error: 'Invalid cron expression: must be 5 or 6 fields' };
    }
...
function registerAuditHandlers() {
  const AUDIT_LOG = path.join(os.homedir(), ".lodestone", "tool-audit.log");

  ipcMain.handle("get-audit-log", async (_e, opts = {}) => {
    const { limit = 200, offset = 0, tool, from, to } = opts;
    try {
      if (!fs.existsSync(AUDIT_LOG)) return { entries: [], total: 0 };
      const data = fs.readFileSync(AUDIT_LOG, "utf-8");
      ...
      console.error("[Audit] Error reading audit log:", err.message);
  ...
  ipcMain.handle("get-audit-stats", async () => {
    try {
      if (!fs.existsSync(AUDIT_LOG)) return { toolCounts: [], total: 0, today: 0, thisWeek: 0 };
      const data = fs.readFileSync(AUDIT_LOG, "utf-8");
      const lines = data.trim().split("\n").filter(Boolean);
      ...
      for (const e of entries) {
        ...
        if (new Date(e.timestamp) >= weekStart) thisWeek++;
      }

      return {
        toolCounts: Object.entries(toolCounts).map(([tool, count]) => ({ tool, count })).sort((a, b) => b.count - a.count),
        total: entries.length,
        today,
        thisWeek
      };
    } catch (err) {
      console.error("[Audit] Error reading audit stats:", err.message);
      return { toolCounts: [], total: 0, today: 0, thisWeek: 0 };
    }
  });
}
