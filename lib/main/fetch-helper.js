...
function fetchWithNode(url, method = "GET", body = null, headers = {}, timeoutMs = 0, _redirects = 0) {
  return new Promise((resolve, reject) => {
    ...
    const req = https.request(options, (res) => {
      if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
        ...
        return fetchWithNode(redirectUrl, method, body, headers, timeoutMs, _redirects + 1).then(resolve).catch(reject);
      }
      const chunks = [];
...
function fetchWithNodeStreaming(url, method = "GET", body = null, headers = {}, _redirects = 0) {
  return new Promise((resolve, reject) => {
    const urlObj = new URL(url);
    const options = {
      method,
    ...
    const req = https.request(options, (res) => {
      if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
        ...
        const redirectUrl = res.headers.location.startsWith("http")
          ? res.headers.location
          : new URL(res.headers.location, url).toString();
        return fetchWithNodeStreaming(redirectUrl, method, body, headers, _redirects + 1).then(resolve).catch(reject);
      }
