...
if (!window.__lodestone_data_layer_active) {
  ...
  dlScript.onload = function() { console.debug('[Lodestone] Data layer loaded from external script'); };
...
const DESKTOP_DETECT_SCRIPT = [
  ...
  // All /api/ requests go through the lodestone:// protocol handler.
  // The protocol handler proxies them to https://api.heylodestone.com.
  // Direct https:// fetch is blocked by CORS (Cloudflare caches preflight responses
  // with the wrong origin, and lodestone:// origins aren't in the cached allow list).
  "(function(){",
